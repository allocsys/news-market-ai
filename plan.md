...
## Next To-Dos (2026-09-26)
Priority order, code-verified against current `main` (671123a):
...
3b. ~~D1 free-tier daily row-read limit hit again (2026-09-25)~~ — root cause fixed 2026-09-25 via **PR #126** (merged). Confirmed holding 2026-09-26 after ~1h45min of live traffic post-unpause (10:22Z-11:19Z) with zero "exceeded" errors; ingestion/LLM/trading all producing real activity in the same window (real ingest fetches, exit_check completions every ~15min cron tick, one real position closed on stop_loss). Recommend watching through at least one full day-cycle before fully declaring resolved. See 2026-09-26 pause/unpause note below.
4. **Verify `SIM_DB` purge-guard actually fires in prod** — binding and guard code are in place (`ingest-worker.js`, `wrangler.ingest.toml`), just never confirmed against a real purge run. No longer blocked by 3b (read-limit issue fixed); now blocked on backtests being unpaused (2026-09-26 pause/unpause note below), since the guard only matters while a backtest is queued/running.
5. **First clean backtest (F)** — 2 complete / 11 failed / 3 cancelled in `sim.backtest_runs` as of 2026-09-24; still unreviewed whether either complete run counts. No longer blocked by 3b (read-limit issue fixed); currently blocked by the owner's backtest pause (2026-09-26, see below) — resume once owner unpauses backtests.
6. ~~`BACKTEST_DAILY_WRITE_BUDGET`~~ — BUILT 2026-09-25 on `feature/backtest-daily-write-budget`: `config.js` (`BACKTEST_DAILY_WRITE_BUDGET`, default 40000, 0 disables), `sim_registry.js` accumulator/reader, `subrequestBudget.js` rows-written tracking, `backtest-worker.js` enforcement (refuses a continuation once today's cross-run total is at/over budget; part 1 of a new run is never refused), migration `sim/0002`, full test coverage. 40K confirmed (owner, 2026-09-25) as a self-imposed sub-limit within the 100K/day platform cap (Free-plan budgets, below). **PR #127 merged 2026-09-25.**
7. **Portfolio correlation/cross-asset-exposure check** — still just a flat risk-budget check (`portfolio_manager.js`). **DECIDED 2026-09-25 (owner): build the computed price-return correlation matrix, not a static sector map.** Rationale: the static map would misclassify exactly the cases that matter (XAUUSD/USO both macro-risk-off-correlated despite "different" buckets; AAPL/MSFT/TSLA not always correlated despite "same" bucket), and a manually-asserted grouping conflicts with Adopted Pattern #9 (grounded data claims, nothing from memory/assertion). Scoping: buildable on existing plumbing, no new adapters needed --
    ...
    - Integration point: `portfolio_manager.js`'s `evaluatePortfolio` (same placeholder-comment location as `MAX_PORTFOLIO_RISK_PCT`) -- extend the flat risk-budget check with a correlation-weighted exposure check before sign-off.
    - Untuned like everything else here until backtest history exists to validate a threshold against; ship behind the same untuned-placeholder convention as `MAX_PORTFOLIO_RISK_PCT`, not gated on perfect tuning.
    - **Blocked on #5 / backtests being unpaused** — validating a correlation-weighted exposure check needs backtest history to test against, so this is logically next-in-line once backtests come back.
8. ~~Drop dead `debate_id` column/field~~ — BUILT 2026-09-25 on `feature/drop-debate-id-column`: migration `state/0003` drops `trade_decisions.debate_id`; `run_store.js`'s `commitThesis`/`insertTradeDecision`/`listRecentTradeDecisions` no longer read/write it; `schemas/index.js`'s `TradeThesis.debateId` removed. No dashboard or test changes needed -- neither referenced it (checked directly, not just via the pipeline argument above). Mechanical, no behavior change: the column never held a real value in production. **PR #128 merged 2026-09-25.**
9. **NEW FINDING (2026-09-26): the backtest walk's grace period is dead weight for the scoring path actually used.** `onSignalRunner.js#computeWalkEnd` extends the day-by-day walk past `testEnd` by `graceDays` (default `config.maxPositionHoldDays` = 10) so a position still open at `testEnd` gets a fair shot at its stop-loss/take-profit/time exit instead of being cut off mid-trade. That reasoning is real, but only for the *older* realized-per-closed-trade scoring (`onSignalRunner.js#runOnSignalReturns`, feeding `signalCompare.js`) — nothing currently wires that path into a real entry point. The path `runBacktest.js` actually uses (`equity.js#onEquityReturns`) marks every position to market **daily** and **never force-closes** it; a position still open at the end of the scored span simply stays in the curve at its last mark. Its price grid (`loadPriceGrid`, called with `testStart`/`spanEnd` = the requested `testEnd`) never reads a single day past `testEnd` either. Net effect: every extra day the grace period walks — full pipeline run (real Gemini calls: 3 analysts + bull/bear/judge + trader) plus exit checks — happens **entirely outside the window `runBacktest.js` scores**, so it cannot change the result. This is the root cause of the "2-day backtest walked 9 days" confusion (2026-09-26 session) and a direct contributor to burning free-tier Gemini RPM on work that's thrown away. Confirmed by reading `runBacktest.js`, `onSignalRunner.js`, `equity.js` this session; not yet live-verified end to end (e.g. by comparing a run's score with and without grace). **Proposed, not yet built or owner-approved:** skip/zero the grace-period walk specifically for the `runBacktest.js` equity-curve path — compute P&L straight off the price grid without extending the LLM walk past `testEnd`. Would cut both wall-clock time and real Gemini quota on every future backtest run at no cost to the score.

**Done since last pass, removed from backlog:** dashboard `?env=` selector (built — `src/dashboard/views/env_selector.js`, `parseEnvParam`/`resolveEnv`); #125/#126 merged 2026-09-25 (Gemini 3.5-flash fallback added, D1 news-scan bounded by date instead of ticker history, job-progress polling cut 1.5s→5s, `deleteRun` batched); #127 merged 2026-09-25 (`BACKTEST_DAILY_WRITE_BUDGET`); #128 merged 2026-09-25 (dropped dead `trade_decisions.debate_id` column/field).

**System pause/unpause (2026-09-26):** owner paused all system flags (ingestion/trading/llm/backtests) 07:43:52Z. Ingestion/trading/llm unpaused 10:22Z per owner instruction ("all unpaused except backtest"); **backtests remain paused**, untouched, pending the owner's own call on how long live traffic needs to run clean first. Post-unpause health checks (10:22Z-11:19Z, ~1h45min) found zero D1 "exceeded" errors and normal ingest/exit_check/backfill activity — detail in item 3b above.
