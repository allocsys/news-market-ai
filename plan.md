...
7. **Portfolio correlation/cross-asset-exposure check** — still just a flat risk-budget check (`portfolio_manager.js`). Scoped 2026-09-25: no foundation exists yet -- `config.watchlist` is just `{ticker, query}`, no sector/asset-class tags, no return-correlation matrix anywhere in the codebase. Needs a design decision (static sector map vs. computed price-return correlation) before implementation, not just a code change.
