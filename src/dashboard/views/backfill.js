...
export function backfillTriggerForm() {
  ...
  return `<form method="get" action="/dashboard/backfill/confirm" class="filter-bar">
    ...
    <div class="filter-group">
      ...
      <button type="submit">Review &amp; run backfill</button>
    </div>
  </form>`;
}
...
export function renderBackfillConfirmPage({ from, to }) {
  return `<section id="backfill-confirm">
    ...
    <p class="note" style="color: #c1502e; font-weight: 600;">Cost warning: This action makes real Finnhub API calls and spends free-tier quota.</p>
    <form method="post" action="/backfill" class="filter-bar">
      ...
      <div class="filter-group">
        <button type="submit">Confirm and run backfill</button>
      </div>
    </form>
  </section>`;
}
